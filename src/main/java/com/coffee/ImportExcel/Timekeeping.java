...
public class Timekeeping {
    ...
    public Pair<Boolean, String> addWorkChamCongFromExcel(File file) throws IOException {
        ...
        try (FileInputStream fis = new FileInputStream(file); Workbook workbook = new XSSFWorkbook(fis)) {
            ...
            Pair<List<Work_Schedule>, String> Work_SchedulesResult = readTimekeepingFromExcel(sheet1);
            ...
            if (errorAll.isEmpty()) {
                ...
                updateTimekeeping(work_schedules);
            } else {
    ...
    private Pair<List<Work_Schedule>, String> readTimekeepingFromExcel(Sheet sheet) {
            ...
            switch (columnIndex) {
                ...
                case 5: // check_in
                    if (cell.getCellType() == CellType.NUMERIC && DateUtil.isCellDateFormatted(cell)) {
                        // Nếu ô có định dạng thời gian
                        Date date = cell.getDateCellValue();
                        SimpleDateFormat sdf = new SimpleDateFormat("HH:mm");
                        String timeValue = sdf.format(date);
                        rowData.put("check_in", timeValue);
                    }
                ...
                case 6: // check_out
                    if (cell.getCellType() == CellType.NUMERIC && DateUtil.isCellDateFormatted(cell)) {
                        ...
                        SimpleDateFormat sdf = new SimpleDateFormat("HH:mm");
                        String timeValue = sdf.format(date);
                        rowData.put("check_out", timeValue);
                    }
                default:
                    break;
            }
    ...
    private void validateAttributeWork_schedule(Map<String, Object> rowData, StringBuilder errorRow) {
        ...
        if (!rowData.containsKey("staff_id") || rowData.get("staff_id") == null) {
            ...
            if (!staffs.isEmpty())
                // lấy ra tên nhân viên để kiểm tra xem tên nhân viên có đúng với mã nhân viên không
        ...
        if (!rowData.containsKey("check_in") || rowData.get("check_in") == null) {
            errorRow.append("Giờ vào phải theo định dạng HH:mm ví dụ 6:00 .\n");
        }
        if (!rowData.containsKey("check_out") || rowData.get("check_out") == null) {
            errorRow.append("Giờ ra hải theo định dạng  HH:mm ví dụ 6:00 .\n");
        }
    ...
    private void checkExistDatabase(List<Work_Schedule> workSchedules, StringBuilder errorAll) {
        for (int i = 0; i < workSchedules.size(); i++) {
            ...
            Pair<Boolean, String> result = work_ScheduleBLL.isTimed(work_schedule);
            if (!result.getKey())
                errorAll.append("- Dòng ").append(i + 2).append(result.getValue()).append("\n");
        }
    }

    private void updateTimekeeping(List<Work_Schedule> workSchedules) {
        for (Work_Schedule work_schedule : workSchedules) {
            int staff_id = work_schedule.getStaff_id();
            Date date = work_schedule.getDate();
            String checkin = work_schedule.getCheck_in();
            String checkout = work_schedule.getCheck_out();
            int shift = work_schedule.getShift();
            work_ScheduleBLL.updateTimekeepng(staff_id, date, shift, checkin, checkout);
        }
    }
}
